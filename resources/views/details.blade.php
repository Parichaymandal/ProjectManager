	...
	<div class="col-sm-8">
	    <div class=row>
            ...
            <div class="col-sm-5" Style="height:auto ;text-align:center">
                ...
                <div class="progress">
                    <div class="progress-bar progress-bar-warning" role="progressbar" aria-valuenow="{{ $jobs[0]->job['partial_completeness'] }}"
                         aria-valuemin="0" aria-valuemax="100" style="width:{!! $jobs[0]->job->partial_completeness !!}%">
                        {{ $jobs[0]->job['partial_completeness'] }}% Complete
                    </div>
                </div>
                ...
                {{--</div>--}}
                <div>
                    @if(sizeof($jobs) == 0 && $jobs[0]->job->status == 0)

                        <a type="button" class="btn btn-success"style="float:left">Accept</a>
                    @endif
                    <a href="{{route('split.edit',$jobs[0]->job->id)}}" type="button" class="btn btn-danger"style="float:right">Split</a>
                        <a href="{{route('split.show',$jobs[0]->job->id)}}" type="button" class="btn btn-primary btn-xs" style="float:right">Reassign</a>
                </div>
                ...
                @if($key > 0)
                    <div class="col-sm-4">
                        ...
                        <div class="row" style="align-content: center">
                           <a href="{{route('taskdetails.show',$job->id)}}" style="height: 10px" class="btn btn-success">Details</a>
                        </div>
                    </div>
        ...
        <div class="row;_margin">
            ...
            <div class="col-sm-6" Style="min-height:300px; max-height:auto">
                ...
                @foreach($jobs[0]->job->requirements as $requirement)
                    <ul class="panel panel-default">
                        <li class="panel-body">{{$requirement['resuirement']}}</li>
                    </ul>
                @endforeach
            ...
            <div class="col-sm-6 " Style="min-height:300px; max-height:auto" >
                ...
                @foreach($jobs[0]->job->milestones as $milestone)
            ...
            <div class="row" style="height:auto">
                ...
                @if($jobs != null)
                    @foreach($jobs as $j)
                        @if($j->file != null)
                            <div class=col-sm-2>
                                <a href="{{route('submit.show',$j->id)}}" target="_blank"><img id="file" src="/img/file.png" style="height:60px ;width:60px" href="" alt ="no image">{{$j->file}} {{$job->file_uploaded_at}}</a><br>
                                <a href="{{route('submit.show',$j->id)}}" target="_blank"><label for="file" id="label" >{{$j->file}}</label></a>
                            </div>             @endif

                        @if($j->jobs != null)
                            <div class=col-sm-2>
                                <a href="{{route('repogitory.show',$j->id)}}" target="_blank"><img id="image" src="/img/folder.png" style="height:60px ;width:60px" href="" alt ="no image"></a><br>
